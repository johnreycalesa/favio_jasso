    ...
    <div class="flex">
        ...
        <main class="flex flex-col max-w-[1000px] w-full gap-5 p-5">
            <div class="flex items-center justify-between">
                <p class="text-2xl font-extrabold text-orange-700">Availability Table</p>
                <a href="{{ url('admin/availabilities/create') }}" class="p-3 text-white bg-orange-500 rounded-lg">
                ...
                <tbody class="[&>*]:bg-white [&>tr>*]:p-5">
                    {{-- @foreach ($availabilities as $availability)
                        <tr class="w-full text-center [&>*]:border" id="{{ $appointment->id }}">
                            <td>{{ $availability->schedule->service->type }}</td>
                            <td>{{ $availability->schedule->date }}</td>
                            <td>{{ $availability->start_time }}</td>
                            <td>{{ $availability->end_time }}</td>
                            <td>{{ $availability->status }}</td>
                            <td>
                                @if ($availability->status == 'Available')
                                    <a href="{{ url('/admin/availability/update/' . $availability->id) }}"
